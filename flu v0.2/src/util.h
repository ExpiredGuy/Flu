...
#include <vector>
//-------------------------------------------//
const std::string flu_fen_positions[71] = {
...
//-------------------------------------------//
char flu_piece_char(flu_piece_code flu_c, bool flu_lower = false);
std::string flu_move_to_str(flu_move flu_code);
std::vector<std::string> flu_split_string(const char* flu_c);
unsigned char flu_algebraic_to_index(const std::string& flu_s);
void flu_bench();
void* flu_alloc_aligned_mem(size_t flu_alloc_size, void*& flu_mem);
